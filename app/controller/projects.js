...
export function socket(app) {
    app.on("getModules", async (response) => {
        if (!isValidObject(app, response, true)) return app.emit("getModules", {"error": "you must be logged in"});

        let year = response.year;
        let units = [];

        if (response.year === undefined) return app.emit("getModules", {"error": "need `year` parameter"});

        let json = await getAllModules(app.cookie);
    ...
    app.on("getProjects", async (response) => {
        ...
        if (![year, codeModule, codeInstance, codeActivity].every((value) => value !== undefined)) return app.emit("getProjects", {"error": "bad/missing arguments"});
        try {
            activity = await getActivity(year, codeModule, codeInstance, codeActivity, cookies);
        } catch (e) {
            return app.emit("getProjects", {"error": "bad/missing arguments"});
        }
        let project = null;
        let events = [];
    ...
    app.on("getProjectInfo", async (response) => {
        ...
        let codeInstance = response.code_instance;
        let codeActivity = response.code_activity;
        let typeActivity = response.type_activity;
        let json = {};

        if (![year, codeModule, codeInstance, codeActivity, typeActivity].every((value) => value !== undefined)) return app.emit("getProjectInfo", {"error": "bad/missing arguments"});
        try {
            switch (typeActivity) {
                case "project":
                    try {
                        ...
                        return app.emit("getProjectInfo", {"error": "bad/missing arguments"});
                    }
                    let notRegistered = [];
                    ...
                    return app.emit("getProjectInfo",{
                        beginDate: json["begin"],
                ...
                case "rdv":
                    try {
                        ...
                        return app.emit("getProjectInfo", {error: "invalid argument"})
                    }
                    let events = [];
                ...
                case "note":
                    ...
                    return app.emit("getProjectInfo",notes)
                default:
                    return app.emit("getProjectInfo",{error: "invalid argument"})
            }
        } catch (e) {
            console.log(e)
            return app.emit("getProjectInfo",{error: "internal error"})
        }
    })
}
